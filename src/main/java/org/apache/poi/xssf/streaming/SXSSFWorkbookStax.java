 ...
 */
public class SXSSFWorkbookStax extends SXSSFWorkbook implements Workbook {
    /**
     ...
     */
    public SXSSFWorkbookStax(XSSFWorkbook workbook){
     ...
     */
    public SXSSFWorkbookStax(XSSFWorkbook workbook, int rowAccessWindowSize){
     ...
     */
    public SXSSFWorkbookStax(XSSFWorkbook workbook, int rowAccessWindowSize, boolean compressTmpFiles){
     ...
     */
    public SXSSFWorkbookStax(XSSFWorkbook workbook, int rowAccessWindowSize, boolean compressTmpFiles, boolean useSharedStringsTable){
     ...
     * @param rowAccessWindowSize the number of rows that are kept in memory until flushed out, see above.
     */
    public SXSSFWorkbookStax(int rowAccessWindowSize){
                ...
                while (en.hasMoreElements()) {
                    ...
                    if(xSheet!=null && !(xSheet instanceof XSSFChartSheet)) {
                        ...
                        try {
                            ...
                            throw new IOException("Exception in copyStreamAndInjectWorksheet", e);
                        } finally {
                            IOUtils.closeQuietly(xis);
                        }
                    } else {
                        IOUtils.copy(is, zos);
                    }
    ...
    private static void copyStreamAndInjectWorksheet(InputStream in, OutputStream out, InputStream worksheetData) throws IOException, XMLStreamException {
        XMLInputFactory inputFactory = StaxHelper.newXMLInputFactory();
        XMLOutputFactory outputFactory = StaxHelper.newXMLOutputFactory();
        XMLEventReader xer = inputFactory.createXMLEventReader(in);
        try {
            XMLEventWriter xew = outputFactory.createXMLEventWriter(out);
            while(xer.hasNext()) {
                XMLEvent xe = xer.nextEvent();
                xew.add(xe);
                if(xe.isStartElement() && SHEET_DATA.equals(xe.asStartElement().getName().getLocalPart())) {
                    QName parentElementName = xe.asStartElement().getName();
                    injectWorksheet(inputFactory, xew, parentElementName, worksheetData);
                }
            }
            xew.flush();
        } finally {
    ...
    private static void injectWorksheet(XMLInputFactory inputFactory, XMLEventWriter xew, QName parentElementName, InputStream worksheetData)
            throws IOException, XMLStreamException {
        XMLEventFactory eventFactory = StaxHelper.newXMLEventFactory();
        String headerText = String.format(LocaleUtil.getUserLocale(), "<%s>", SHEET_DATA);
        String footerText = String.format(LocaleUtil.getUserLocale(), "</%s>", SHEET_DATA);
        InputStream header = new ByteArrayInputStream(headerText.getBytes(StandardCharsets.UTF_8));
        InputStream footer = new ByteArrayInputStream(footerText.getBytes(StandardCharsets.UTF_8));
        Enumeration<InputStream> en = Collections.enumeration(Arrays.asList(new InputStream[]{header, worksheetData, footer}));
        SequenceInputStream sis = new SequenceInputStream(en);
        XMLEventReader worksheetReader = inputFactory.createXMLEventReader(sis);
        try {
            while(worksheetReader.hasNext()) {
                XMLEvent worksheetEvent = worksheetReader.nextEvent();
                if(worksheetEvent.isStartDocument() || worksheetEvent.isEndDocument()) {
                    // ignore
                } else if(worksheetEvent.isStartElement()) {
                    StartElement se = worksheetEvent.asStartElement();
                    String localName = se.getName().getLocalPart();
                    if(!"sheetData".equals(localName)) {
                        XMLEvent eventWithNamespace = eventFactory.createStartElement(
                                parentElementName.getPrefix(),
                                parentElementName.getNamespaceURI(),
                                se.getName().getLocalPart(),
                                se.getAttributes(),
                                se.getNamespaces());
                        xew.add(eventWithNamespace);
                    }
                } else if(worksheetEvent.isEndElement()) {
                    String localName = worksheetEvent.asEndElement().getName().getLocalPart();
                    if(!"sheetData".equals(localName)) {
                        XMLEvent eventWithNamespace = eventFactory.createEndElement(
                                parentElementName.getPrefix(),
                                parentElementName.getNamespaceURI(),
                                localName);
                        xew.add(eventWithNamespace);
